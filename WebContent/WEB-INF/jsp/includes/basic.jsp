<!-- <script type="text/javascript" src="https://code.jquery.com/jquery-1.12.4.js"></script> -->
<!-- <script type="text/javascript" src="https://cdn.datatables.net/1.10.13/js/jquery.dataTables.min.js"></script> -->


<!-- JQUERY -->
<script type="text/javascript" src="../js/commons/jquery-2.2.4.min.js"></script>
<script type="text/javascript" src="../js/commons/jquery-ui.min.js"></script>

<!-- BOOTSTRAP -->
<script type="text/javascript" src= "../js/commons/bootstrap/tether.min.js"></script>
<script type="text/javascript" src= "../js/commons/bootstrap/bootstrap.min.js"></script>
		
<!-- DATATABLES -->		
<script type="text/javascript" src="../js/commons/datatables/datatables.min.js"></script>

<!-- PEACE.JS -->		
<script type="text/javascript" src="../js/commons/peace.min.js"></script>

<!-- BOOTBOX.JS -->		
<script type="text/javascript" src="../js/commons/bootbox.min.js"></script>

<!--  LOCAL - JS -->
<script type="text/javascript" src="../js/pes.js"></script>

<!-- CSS - GENERAL -->
<link rel="stylesheet" href="../css/commons/pes.css" />
<link rel="stylesheet" href="../css/commons/jquery-ui.min.css" />

<!-- CSS - BOOTSTRAP -->
<link rel="stylesheet" href="../css/commons/bootstrap/bootstrap.min.css" />

<!-- CSS - DATATABLES -->
<link rel="stylesheet" href="../css/commons/datatables/dataTables.bootstrap4.min.css" />
<link rel="stylesheet" href="../css/commons/datatables/datatables.min.css" />

<!-- CSS - PEACE -->
<link rel="stylesheet" href="../css/commons/peace-theme-loading.css" />